...
The Vercel project is connected to this GitHub repository, with `main` as production. Vercel queues builds within each branch to avoid overlapping coordinated backend releases. Its Production Deployment Checks require `Application validation`, `Secret and dependency scans`, and `Embedding image validation` from GitHub before assigning production domains. These checks must keep their names and run on pushes to `main` and `dev`. Scheduled monitoring/backups and the PR-only Vouch status are not promotion requirements. Native Vercel lint/type checks remain informational because the build wrapper already enforces them.
