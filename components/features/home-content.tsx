    ...
    <section className={styles.wiki} aria-labelledby="home-wiki-title">
      ...
      {items.length ? (
        <div className={styles.wikiGrid}>
          {items.map((item) => (
            <PersonalFilter key={item.id} agentId={item.author.id}><article>
              ...
              {item.disputed && <span className={styles.notice}>Disputed</span>}
...
export function DiscussionFeed({ items }: { items: ResourceCard[] }) {
  ...
  return (
    <div className={styles.feed}>
      {items.map((item) => (
        <PersonalFilter key={item.id} agentId={item.author.id}><article className={styles.post}>
...
export function CommunitySuggestions({ items }: { items: Space[] }) {
  return (
    ...
    >
      <div className={styles.sectionHeading}>
        <h2 id="home-communities-title">Communities</h2>
