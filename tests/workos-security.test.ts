...
vi.mock("@workos-inc/node", () => ({
  WorkOS: class {
    ...
    agents = {
      validateCredential: provider.validate,
      getRegistration: provider.registration,
    }
...
const authenticate = (t: Test, n = 0) =>
  t.action(internal.workos.authenticate, { token: token(n) })
const http = (t: Test, n = 0, path = "me/billing") =>
  t.fetch(`/api/v1/${path}`, {
...
describe("WorkOS shared authentication cost controls", () => {
  ...
  it("admits at most the aggregate cap when attempts arrive concurrently", async () => {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 12 }, (_, n) => authenticate(t, n))
    )
  ...
  it("applies the documented default and maximum configuration caps", async () => {
    for (const [setting, maximum] of [
      ...
      expect(
        (await t.run((ctx) => ctx.db.query("limits").unique()))?.count
      ).toBe(maximum)
    }
    expect(provider.validate).toHaveBeenCalledTimes(2)
  })
  ...
  it("rejects malformed credentials locally without consuming the external-work budget", async () => {
    ...
    for (const bad of [
      ...
      await expect(
        t.action(internal.workos.authenticate, { token: bad })
  ...
  it("revalidates successful credentials and current registration on every request", async () => {
    ...
    expect(provider.validate).toHaveBeenCalledTimes(2)
    expect(provider.registration).toHaveBeenCalledTimes(2)
    expect(provider.validate).toHaveBeenLastCalledWith({
      type: "access_token",
      credential: token(),
      audience: "client_test",
  ...
  it("counts provider errors without leaking their details and leaves legacy/public reads available", async () => {
    ...
    const t = setup()
    provider.validate.mockRejectedValue(
      Object.assign(new Error("sensitive-provider-detail"), { status: 503 })
    )
    ...
    const registered = await t.fetch("/api/v1/agents", {
      ...
      body: JSON.stringify({
        name: "Legacy security fixture",
        slug: "legacy-security-fixture",
      }),
    })
    ...
    expect(
      (
        await t.fetch("/api/v1/me/billing", {
