...
async function human(t: Test) {
  const user = await t.mutation(components.betterAuth.adapter.create, {
    input: {
      ...
      data: {
        ...
        email: `capacity-${++sequence}@example.com`,
        emailVerified: true,
        createdAt: Date.now(),
        updatedAt: Date.now(),
      },
  ...
  const session = await t.mutation(components.betterAuth.adapter.create, {
    input: {
      ...
      data: {
        userId: user._id,
        token: `fixture-session-${sequence}`,
...
async function deposit(
  owner: Human,
  amountCents: number,
  idempotencyKey = `deposit-${++sequence}`
) {
  const quote = sandboxProvider.quote("deposit", amountCents)
  return owner.client.mutation(api.placeWallet.manage, {
    operation: "deposit",
    amountCents,
...
const paymentId = (result: { paymentId?: string }) =>
  result.paymentId as Id<"placePayments">
const bank = (t: Test, owner: Human) =>
  t.run((ctx) =>
    ctx.db
      .query("placeAccounts")
      .withIndex("by_owner", (q) => q.eq("ownerId", owner.id))
      .unique()
  )
...
async function balanced(t: Test) {
  for (const row of await t.run((ctx) => ctx.db.query("placeLedger").collect()))
    expect(
      row.postings.reduce(
        (sum, posting) => sum + BigInt(posting.cents),
        BigInt(0)
      )
    ).toBe(BigInt(0))
}
...
it("rejects aggregate pending deposits before creating excess obligations", async () => {
  ...
  expect(
    await t.run((ctx) => ctx.db.query("placePayments").collect())
  ).toHaveLength(1)
})
...
it("parks a permanent credit-capacity failure instead of scheduling endless retries", async () => {
  const t = setup(),
    owner = await human(t),
    pending = await deposit(owner, 100)
  ...
  expect((await t.run((ctx) => ctx.db.get(paymentId(pending))))?.status).toBe(
    "parked"
  )
  expect(
    (await owner.client.query(api.placeWallet.current, {})).unallocated
  ).toBe(MAX_MONEY)
})
...
it("caps automatic attempts while retaining an uncertain payment obligation", async () => {
  const t = setup(),
    ...
    pending = await deposit(owner, 100)
  const provider = vi
    .spyOn(sandboxProvider, "reconcile")
    .mockRejectedValue(
  ...
  expect(
    await t.run((ctx) => ctx.db.query("placeLedger").collect())
  ).toHaveLength(0)
})
...
it("serializes deposit capacity reservations and keeps receipt/event retries idempotent", async () => {
  ...
  const outcomes = await Promise.allSettled([
    deposit(owner, 600_000_000_000, "first"),
    deposit(owner, 600_000_000_000, "second"),
  ])
  expect(
  ...
  if (accepted.status !== "fulfilled")
    throw new Error("Expected a reserved deposit")
  expect(
    await deposit(owner, 600_000_000_000, winner === 0 ? "first" : "second")
...
it("prevents allocation returns from consuming pending deposit capacity", async () => {
  ...
  const request = {
    operation: "return_funds" as const,
    agentId,
    amountCents: 1000,
    idempotencyKey: "return",
  }
  await expect(
    owner.client.mutation(api.placeWallet.manage, request)
  ).rejects.toThrow(/capacity/i)
  await t.mutation(internal.placeWallet.applyEvent, {
    event: await event(t, paymentId(first), "failed"),
  })
...
it("never refunds an exhausted uncertain withdrawal and accepts its later confirmed callback", async () => {
  ...
  const pending = await owner.client.mutation(api.placeWallet.manage, {
    ...
    amountCents: 500,
    quotedFeeCents: 25,
    idempotencyKey: "uncertain-withdrawal",
  })
  const provider = vi
    .spyOn(sandboxProvider, "reconcile")
    .mockRejectedValue(
      Object.assign(new Error("uncertain provider result"), { status: 503 })
    )
  for (let i = 0; i < 10; i++) {
    await t.action(internal.placeWallet.process, {
      paymentId: paymentId(pending),
  ...
  await expect(
    t.mutation(internal.placeWallet.applyEvent, {
      event: { ...callback, outcome: "succeeded" },
    })
...
it("requires an operator to resume parked work and reuses the recorded provider outcome", async () => {
  ...
  const request = {
    paymentId: paymentId(pending),
    idempotencyKey: "operator-reconcile",
  }
  await expect(
    owner.client.mutation(api.placeWallet.reconcilePayment, request)
  ).rejects.toThrow(/operator/i)
  await expect(
    owner.client.query(api.placeWallet.reconciliationQueue, {})
  ).rejects.toThrow(/operator/i)
  vi.stubEnv("PLACE_OPERATOR_OWNER_IDS", owner.id)
  expect(
    (
      await owner.client.query(api.placeWallet.reconciliationQueue, {})
    ).payments.map((row) => row._id)
  ).toEqual([paymentId(pending)])
  await owner.client.mutation(api.placeWallet.manage, {
    operation: "allocate",
    agentId,
    amountCents: 100,
    idempotencyKey: "free-room",
  })
  vi.stubEnv("PLACE_ENABLED", "false")
  const first = await owner.client.mutation(
    api.placeWallet.reconcilePayment,
    request
  )
  expect(
    await owner.client.mutation(api.placeWallet.reconcilePayment, request)
  ).toEqual(first)
  await t.action(internal.placeWallet.process, {
    paymentId: paymentId(pending),
  })
...
it("preserves overcommitted legacy totals exactly and releases only a confirmed obligation", async () => {
  ...
  const fixtures = await t.run(async (ctx) => {
    const accountId = await ctx.db.insert("placeAccounts", {
      ownerId: owner.id,
      mode: "sandbox",
      currency: "USD",
      unallocated: 0,
    ...
    for (let n = 0; n < 2; n++)
      ids.push(
        await ctx.db.insert("placePayments", {
          ownerId: owner.id,
          kind: "deposit",
          amountCents: 600_000_000_000,
          feeCents: 17_400_000_030,
          status: "pending",
          reference: `legacy-large-${n}`,
          nextAt: Date.now(),
          attempts: 0,
        })
      )
  ...
  await t.mutation(internal.placeWallet.migrateCapacity, {
...
it("migrates all legacy obligations with a persisted bounded cursor before allowing credits", async () => {
  ...
  const fixtures = await t.run(async (ctx) => {
    ...
    const pending = await ctx.db.insert("placePayments", {
      ownerId: owner.id,
      kind: "deposit",
      amountCents: 100,
      feeCents: 33,
    ...
    for (let i = 0; i < 101; i++)
      await ctx.db.insert("placePayments", {
        ...
        feeCents: 33,
        status: "succeeded",
        reference: `legacy-terminal-${i}`,
        nextAt: Date.now(),
        attempts: 1,
      })
    const withdrawal = await ctx.db.insert("placePayments", {
      ownerId: owner.id,
      kind: "withdrawal",
      amountCents: 150,
      feeCents: 25,
      status: "pending",
      reference: "legacy-last",
      nextAt: Date.now(),
      attempts: 0,
    })
  ...
  await t.action(internal.placeWallet.process, { paymentId: fixtures.pending })
  await t.mutation(internal.placeWallet.applyEvent, {
    event: await event(t, fixtures.withdrawal, "failed"),
  })
  expect((await bank(t, owner))?.unallocated).toBe(475)
  expect((await bank(t, owner))?.pendingCapacityCents).toBe("0")
  await balanced(t)
...
it("records a late legacy callback during migration and never replaces its confirmed failed outcome", async () => {
  const t = setup(),
    owner = await human(t)
  const fixtures = await t.run(async (ctx) => ({
    accountId: await ctx.db.insert("placeAccounts", {
      ownerId: owner.id,
      mode: "sandbox",
      currency: "USD",
      unallocated: 475,
    ...
    paymentId: await ctx.db.insert("placePayments", {
      ownerId: owner.id,
      kind: "withdrawal",
      amountCents: 500,
      feeCents: 25,
      status: "processing",
      reference: "legacy-refund",
      nextAt: Date.now(),
      attempts: 1,
    }),
  }))
  ...
  vi.stubEnv("PLACE_OPERATOR_OWNER_IDS", owner.id)
  await owner.client.mutation(api.placeWallet.reconcilePayment, {
